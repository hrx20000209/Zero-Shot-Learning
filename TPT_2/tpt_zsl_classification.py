...
def test_time_tuning(model, inputs, optimizer, scaler, args):
    ...
    for j in range(args.tta_steps):
        with torch.cuda.amp.autocast():
            ...
            # original_img = inputs[0]
            # save_image(original_img, f'original.jpg')
            # selected_inputs = inputs[selected_idx]
            # for i in range(selected_inputs.size(0)):
            #     # selected_img = Image.fromarray(selected_inputs[i].cpu().numpy().astype('uint8')).convert('RGB')
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if not args.cocoop:  # no need to reset cocoop because it's fixed
            ...
            output = test_time_tuning(model, images, optimizer, scaler, args)
        else:
            with torch.no_grad():
                with torch.cuda.amp.autocast():
